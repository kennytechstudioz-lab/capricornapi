...
const NOTIFICATION_TEMPLATES = [
  ...
  },
  {
    ...
    content:
      "Hello {{username}}, your account verification was not approved. Reason: {{reason}}. Please review your details and resubmit.",
  },
  {
    name: "verification_processing",
    title: "Verification Under Review",
    content:
      "Hello {{username}}, thanks for the effort of verifying your {{company_name}} account. Your verification is currently in review and will take 24 hours for review completion, you will be notified upon approval.",
  },
  {
...
const TERMS_SECTIONS: { category: "terms" | "policy"; content: string }[] = [
  {
    category: "terms",
    content:
...
export async function seedNotificationTemplates() {
  for (const tpl of NOTIFICATION_TEMPLATES) {
    const exists = await NotificationTemplate.findOne({ name: tpl.name });
    if (!exists) {
...
export async function seedTermsAndPolicy() {
  const existing = await Term.countDocuments();
  if (existing > 0) return; // Only seed if collection is empty
  await Term.insertMany(TERMS_SECTIONS);
...
export async function seedProjectBlogs() {
  const existing = await Blog.countDocuments({ category: "Project" });
  if (existing > 0) return; // Only seed if no project blogs exist
  await Blog.insertMany(PROJECT_BLOGS);
...
export async function seedReviews() {
  const existing = await Review.findOne({ fullName: "James Whitfield" });
  if (existing) return;
...
export async function seedExecutiveStaff() {
  for (const member of SEED_STAFF) {
    await Staff.updateOne({ name: member.name }, { $set: member }, { upsert: true });
  }
